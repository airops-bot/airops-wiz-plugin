...
**Inputs:** topic (required), audience (default general), word_count (default 1500). Resolve from $ARGUMENTS: first positional = topic; then key=value for audience, word_count. Replace `{topic}`, `{audience}`, `{word_count}` in each step's instructions with these values.
